...
import 'package:handmade/feather/pages/Admin/presention/widgets/widgets_user/cust_itmes_favprodect_user%20.dart';
...
class _FavourateprodectScreenState extends State<FavourateprodectScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MyBloc()..getfavourateProducttouser(),
      child: BlocConsumer<MyBloc, MyState>(
          listener: (context, state) {},
          builder: (context, state) {
            var bloc = MyBloc.get(context);
            return Scaffold(
                appBar: customAppBar("My Favourate", false, context),
                body: Padding(
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, int index) {
                      return CardItemsFavProdectUser(
                        isShowDeleteButton: true,
                        image: bloc.getfavprodect[index]['prodectImage'],
                        name: bloc.getfavprodect[index]['prodectname'],
                        price: bloc.getfavprodect[index]['price'],
                        id: bloc.getfavprodect[index]['prodectuid'],
                      );
                    },
                  ),
                ));
